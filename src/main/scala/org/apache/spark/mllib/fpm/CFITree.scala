...
/**
  ...
  * @tparam T item type
  */
private[fpm] class CFITree[T] extends Serializable {
  ...
  /** Adds a transaction with count which checking closedness */
  def add(t: Iterable[T], count: Long = 1L): this.type = {
    require(count > 0)
    var curr = root

    val all_trans = getTransactions(curr).map(x => (x._1.toSet,x._2 ) )

    for (all_t <- all_trans){
      if((t.toSet).subsetOf(all_t._1)){
        return this
      }
    }
...
//  }
  ...
  def merge(other: CFITree[T]): this.type = {
  ...
  private def project(suffix: T): CFITree[T] = {
    val tree = new CFITree[T]
